...
#include <tmacros.h>
#undef print_time
#define platform_printf(f, ...) rtems_printf(&rtems_test_printer, __VA_ARGS__)
#endif /* __rtems__ */

#define platform_printf(f, fmt, args...) printf(fmt, ##args)
...
static REAL second(void) {
  ...
  TickType_t ccount = xTaskGetTickCount();

  return ((double)ccount) / 1000.0;
}
